const updateProjectBt = document.getElementById(
...
logoutBt?.addEventListener("click", (e: Event) => {
  e.preventDefault();
...
async function prePopulateProjectDetails(id: number, e: Event) {
  ...
  (document.querySelector("#project-id") as HTMLInputElement).value =
    id.toString(); //hidden input field to pass the id to update method :)
...
async function updateProject() {
  ...
  let projectID = (document.querySelector("#project-id") as HTMLInputElement)
    .value;
...
async function showUsersSelectionOptions() {
  ...
  let unassUsers = allUsers.filter((user: any) => {
    return (
      !allProjects.some((project: any) => {
        return project.userAssigned == user.userEmail;
      }) && user.role !== "admin"
    );
  });
  unassUsers.forEach((item: { userEmail: string; id: number }) => {
    html += `<option value = "${item.userEmail}">${item.userEmail}</option>`;
  });

  const userSelection = document.getElementById(
    "user-name"
