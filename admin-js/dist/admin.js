...
logoutBt === null || logoutBt === void 0 ? void 0 : logoutBt.addEventListener("click", (e) => {
    e.preventDefault();
...
function showUsersSelectionOptions() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const resp = yield fetch("http://localhost:3000/projects");
        const allProjects = yield resp.json();
        let html = ``;
        if (allUsers.length === 0) {
            html = `<h3> No Users </h3>`;
        }
        let unassUsers = allUsers.filter((user) => {
            return (!allProjects.some((project) => {
                return project.userAssigned == user.userEmail;
            }) && user.role !== "admin");
        });
        unassUsers.forEach((item) => {
